...
**问题。** worker 上跑着三类由它启动的负载——模型实例、基准测试、共享缓存服务实例——微调任务在计划中。容器执行层已经统一(`gpustack_runtime` 的 `WorkloadPlan` 加 Docker/Podman/K8s deployer),但 **worker 侧的控制回路每类各写一遍**:事件监听、周期对账、卡死自愈、崩溃重启退避、状态写回及其重试、端口分配、日志通道、孤儿容器回收。

代价不在代码量,在于**每类负载各自踩一遍同样的坑**。缓存服务上线时修的那批问题(事件丢失、写回竞态、级联删除回收滞后、非优雅终止),模型实例那边有的修过、有的没有。加第四类就再来一轮。

更深的问题在数据模型:领域语义、执行语义、分布式拓扑挤在同一行,于是调度器、worker、供给子进程、控制器都写它。而写回是"读整行 / 改一字段 / 写回整行",两个写者必然互相覆盖。这已经造成过实例**永久卡在 STARTING**:分布式的从属节点只在启动时上报一次 RUNNING,那一次被主 worker 的整行回写抹掉,此后再无人重报。

**解决方案。** 引入通用 `Workload` 资源,只承载执行语义(spec / binding / status)。领域资源由各自的 controller **编译**成 Workload,再把执行状态**聚合**回领域状态。worker 只 watch 这一种资源,维护一套状态机。用户面 API 不变。
...
- **服务端一次性编译出完整 spec。** 原设想是 controller 把 Workload 编译到可直接执行。有五处依赖只有 worker 知道——本机加速器后端与运行时版本、镜像回退仓库、端口占用、本地路径、设备插件的实际分配——所以 spec 分两段:controller 负责与节点无关的部分,worker 启动时补齐其余。

- **执行状态双向一一映射。** 领域状态比执行状态丰富:`ANALYZING` / `SCHEDULED` / `DOWNLOADING` 发生在容器存在之前,而 `INITIALIZING` 与领域自己的 `STARTING` 都对应"已拉起、未健康"。**正向是全映射,反向只有 3/6**。反向不是函数,因此聚合在这些位置**弃权**而非猜测——启动阶段的生命周期仍由领域资源自己写。

- **用一把锁解决写入乱序。** "写领域资源 + 写行"两次调用之间会被别的写者插入,但进程内锁解决不了:供给子进程是**另一个进程**,而读取方在服务端。改为由服务端在写领域资源的同一事务内派生行——不是防住乱序,是让乱序不可能。
...
```
              我们做的            谁调度              binding 从哪来
Docker 集群    资源需求估算   →   gpustack 调度器   →  调度器写入
k8s 集群       换算成 credit  →   Kueue 准入 + k8s  →  从 Pod / CRD 观测回写
```

模型需要多少显存是领域知识(与后端、量化、上下文长度有关),k8s 不懂;而哪个节点有空、和别的负载怎么抢,是 k8s 的领域——我们不该在它旁边再算一遍,尤其算不准,因为看不见非 gpustack 的 Pod,包括 GPU instance 的。
...
| 场景 | 变更前 | 变更后 |
|---|---|---|
| 缓存服务实例日志 | PENDING 下无查看入口 | **可查看**。拉镜像发生在 PENDING、可达数分钟,供给日志从启动那刻就存在 |
| 缓存服务删除后的容器回收 | 最长约 7 分钟 | **秒级** |
...
| 4.3 | k8s:换算 credit、接入 Kueue、从 Pod / CRD 观测回写 binding | 4.2 | 是(按集群类型) |
...
`group_key` 对应 gang;`INITIALIZE_LATER`(从属等主节点初始化后再启动)这类启动顺序协调,在 k8s 上交给 JobSet / LeaderWorkerSet 与 Kueue 的 gang 准入,不再由 worker 互相等待。Docker 集群维持现状。
