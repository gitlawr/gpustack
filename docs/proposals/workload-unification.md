...
| 生态集成 | 不做 | gang 调度、配额、编排原语 |

这条定位是后文所有分工的前提。**它意味着两种集群不必在能力上对齐**:k8s 上该用生态里成熟的东西,Docker 上则保持轻量、够用即可,不为了对称而在 Docker 上重造一遍队列或 gang 调度。

反过来也成立:凡是只在 k8s 生态存在的能力,不构成 Docker 集群的缺口。
...
这个分工直接来自开头的定位。模型需要多少显存是领域知识(与后端、量化、上下文长度有关),k8s 不懂,所以评估始终归我们;而哪个节点有空、和别的负载怎么抢,是 k8s 的领域——在面向生产的集群上,我们不该在它旁边再算一遍,尤其算不准,因为看不见非 gpustack 的 Pod,包括 GPU instance 的。
