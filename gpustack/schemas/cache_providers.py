...
def _validate_component_shape(name: str, component: CacheProviderComponent) -> None:
    ...
    # The address has to survive every configuration: a gated address port
    # would leave the component unaddressable for the ones that turn it off.
    address_port = component.address_port_name()
    if address_port not in component.enabled_ports({}):
        raise ValueError(
            f"component '{name}' takes its address from port "
            f"'{address_port}', which some configurations do not bind"
        )
...
class CacheProvider(BaseModel):
    ...
    def enabled_port_names(
        ...
        given configuration."""
        return self._port_layout(component).enabled_ports(config_fields)
