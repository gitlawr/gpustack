...
_TO_INSTANCE_STATE = {
    # No entry for STARTING: the instance's own lifecycle keeps writing the
    # way up, and _HOLD below turns a starting leader away before it gets
    # here. The state is reachable on a row, just not one the fold speaks for.
...
"""The states a subordinate worker is ever put in.

Narrower than the instance's own: a subordinate is never analysed, scheduled
or downloading -- those describe preparing the instance, which happens once
for the group. Every one of these maps to a distinct workload state, so unlike
the instance's full lifecycle this one survives the round trip.
"""

_TO_SUBORDINATE_STATE = {
    WorkloadStateEnum.PENDING: ModelInstanceStateEnum.PENDING,
    WorkloadStateEnum.STARTING: ModelInstanceStateEnum.INITIALIZING,
    WorkloadStateEnum.RUNNING: ModelInstanceStateEnum.RUNNING,
    WorkloadStateEnum.UNREACHABLE: ModelInstanceStateEnum.UNREACHABLE,
    WorkloadStateEnum.ERROR: ModelInstanceStateEnum.ERROR,
}
"""The inverse of :data:`_TO_WORKLOAD_STATE` over :data:`SUBORDINATE_STATES`.

A separate table from :data:`_TO_INSTANCE_STATE`, which answers a different
question: what the whole instance should say. That one has no entry for
pending or starting because the instance reaches a workload's pending from
five states and the way up belongs to the domain resource. A subordinate
reaches it from one, so here it round-trips.
"""


def to_subordinate_state(
    state: Optional[WorkloadStateEnum],
) -> Optional[ModelInstanceStateEnum]:
    """A follower row's state, in the spelling the subordinate list uses.

    None for a state no subordinate is put in -- succeeded, which only task
    workloads reach -- rather than a guess, so a caller comparing the two
    readings skips it instead of reporting a difference it cannot act on.
    """
    return _TO_SUBORDINATE_STATE.get(state)
