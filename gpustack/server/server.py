...
class Server:
    ...
    async def start(self):
        ...
        # Initialize coordinator (will use LocalCoordinator if no plugin provides one)
        await self._init_coordinator()

        self._run_migrations()
        await self._prepare_data()

        # Preload change-detection cache after the DB is ready. Required in
        # distributed mode so the first cross-instance event on each topic
        # carries accurate ``changed_fields``.
    ...
    async def _init_coordinator(self):
        """Initialize coordinator from extension plugin or use local coordinator."""
        coordinator = None

        for name, plugin in iter_plugins():
            try:
                coordinator = await plugin.create_coordinator(self._config)
                if coordinator:
                    logger.info(f"Coordinator created by plugin: {name}")
                    break
            except Exception as e:
                raise RuntimeError(
                    f"Failed to create coordinator from plugin '{name}': {e}"
                ) from e
