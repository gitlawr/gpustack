...
class ServeManager:
    ...
    def sync_model_instances_state(self):  # noqa: C901
        ...
        # during a reconnect window sees an empty-but-"authoritative" cache; a
        # cache-miss fallback can also truncate at the default perPage=100.
        # Both surface healthy instances as stale. So confirm any reap
        # candidates against a fresh, unpaginated, uncached fetch — this only
        # touches the DB when something actually looks reapable.
        if reap_ids:
            response = self._clientset.model_instances.list(
                params={"page": -1},
                use_cache=False,
