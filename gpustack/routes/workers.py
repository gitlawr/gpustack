...
def _make_worker_visibility_filter(ctx):
    ...
    def _visible(w) -> bool:
        if _bypass_tenant_filter(ctx):
            return True
        org_id = getattr(w, "organization_id", None)
        if (
...
async def get_workers(
    ...
    # Worker carries denormalized organization_id (synced from cluster) so
        ...
        to_create_worker = Worker.model_validate(
            {
                **worker_in.model_dump(exclude={"name", "worker_uuid"}),
                ...
                "state": WorkerStateEnum.READY,
                "organization_id": (
                    cluster.organization_id if cluster is not None else None
                ),
                **kwargs,
            }
        )
