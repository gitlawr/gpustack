...
def _can_use_all_scope(user: User, ctx) -> bool:
    ...
    those rows carry the model owner's organization_id, not the
    Personal Org's.
    """
    if user.is_admin:
        return True
    ...
    if ctx.org_role != OrgRole.ADMIN:
        return False
    return not ctx.current_org_is_personal
...
def _apply_usage_scope_and_filters(
    ...
    *,
    ...
    # view restricts by organization_id when one is in context (platform
