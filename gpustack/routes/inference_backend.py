...
async def list_backend_configs(  # noqa: C901
    ...
    try:
        ...
        #   (InferenceBackendListItem) has no organization_id field to
        ...
        if bypass_filter:
            visible_rows = [b for b in all_rows if b.organization_id is None]
        else:
            visible_rows = [
                b
                for b in all_rows
...
def _collapse_by_backend_name(
    ...
    for backend in db_result_sorted:
        ...
        if backend.organization_id is not None:
...
async def create_inference_backend(
    ...
    """
    target_org_id = getattr(backend_in, "organization_id", None)
    validate_org_owned_owner(
        target_org_id,
        ctx,
...
async def create_inference_backend_from_yaml(  # noqa: C901
    ...
    try:
        ...
        target_org_id = req_yaml_data.get("organization_id")
        validate_org_owned_owner(
            target_org_id,
            ctx,
            resource_label="inference backend",
