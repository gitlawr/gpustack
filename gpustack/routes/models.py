...
async def get_models(
    ...
    # default Org.
    if ctx.current_org_id is not None and not bypass_tenant_filter(ctx):
...
async def create_model(
    ...
    org_scope = ctx.current_org_id
    existing = await Model.one_by_fields(
        session,
        {"name": model_in.name, "organization_id": org_scope},
    )
    if existing:
    ...
    # Stamp tenant scope. ModelBase has organization_id defaulted to
    # PLATFORM_ORGANIZATION_ID, so `model_dump()` always emits the key —
