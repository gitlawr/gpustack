...
class WorkloadClient:
    def __init__(self, client: HTTPClient, enable_cache: bool = True):
        ...
        self._cache: Dict[int, WorkloadPublic] = {}
    ...
    def list(
        ...
        return WorkloadsPublic.model_validate(response.json())

    def _list_from_cache(
        self, params: Dict[str, Any] = None
    ) -> Optional[WorkloadsPublic]:
        """
        ...
        return WorkloadsPublic(items=all_items, total=total, pagination=pagination)
        ...
        try:
            ...
            if is_id_only_delete:
                ...
                if item is not None:
                    ...
                    event.data = item
                logger.debug(f"Cache: removed workload {event.id}")
                return

            item = WorkloadPublic.model_validate(event.data)
            ...
            with self._cache_lock:
                if event.type == EventType.DELETED:
                    ...
                    logger.debug(f"Cache: removed workload {item.id}")
                else:  # CREATED or UPDATED
                    self._cache[item.id] = item
                    logger.trace(f"Cache: updated workload {item.id}")
        except Exception as e:
            logger.error(f"Failed to update workloads cache from event: {e}")
                    ...
                    try:
                        ...
                        if line:
                            ...
                            if self._enable_cache:
                                ...
                                if (
                                    ...
                                    if cache_size > 0:
                                        ...
                                        logger.debug(
                                            f"workloads cache populated with {cache_size} items"
                                        )
                            ...
                            if (
                                ...
                                logger.debug(
                                    f"Skipping callback for ID-only {event.type} event on workloads {event.id}"
                                )
                            elif callback:
    ...
    def get(self, id: int, use_cache: bool = True) -> WorkloadPublic:
        """
        ...
        if use_cache and self._enable_cache:
            with self._cache_lock:
                if self._watch_started and id in self._cache:
                    logger.trace(f"Cache hit for workload {id}")
    ...
    def create(self, model_create: WorkloadCreate):
    ...
    def update(self, id: int, model_update: WorkloadUpdate):
