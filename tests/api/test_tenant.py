...
def _org(is_personal: bool = False):
    org = MagicMock()
...
@pytest.mark.asyncio
async def test_member_uses_default_org():
    user = _user(id=10, is_admin=False, default_organization_id=1)
    request = _request()
    membership = OrganizationMembership(
        user_id=10, organization_id=1, role=OrgRole.USER
    )
    session = _session_returning(
        membership,  # membership lookup
        [5, 6],  # group ids
        [101, 102],  # cluster ids
        _org(is_personal=False),  # org lookup
    )
    ...
    assert ctx.current_org_is_personal is False
...
async def test_api_key_overrides_header():
    ...
    request = _request(api_key=_api_key(organization_id=42))
    membership = OrganizationMembership(
        user_id=10, organization_id=42, role=OrgRole.USER
    )
    session = _session_returning(membership, [], [], _org(is_personal=False))
...
@pytest.mark.asyncio
async def test_personal_org_member_flagged_in_context():
    """Personal Org admins must be distinguishable from real Org admins so
    callers like the Usage page can avoid the org_id filter that would hide
    the user's cross-org Platform usage."""
    user = _user(id=10, is_admin=False, default_organization_id=2)
    request = _request()
    membership = OrganizationMembership(
        user_id=10, organization_id=2, role=OrgRole.ADMIN
    )
    session = _session_returning(
        membership,
        [],
        [],
        _org(is_personal=True),
    )
    ...
    assert ctx.current_org_is_personal is True
    assert ctx.org_role == OrgRole.ADMIN
