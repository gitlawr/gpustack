...
def _ctx_for(user):
    ...
    Admins resolve with no current_org_id (cross-Org "All" mode); regular
    users carry their default Org so they read their own usage rows. The
    route only touches ``is_platform_admin`` / ``current_org_id`` /
    ``user``, so a MagicMock with those fields is enough."""
    ctx = MagicMock()
    ctx.user = user
    ctx.is_platform_admin = bool(getattr(user, "is_admin", False))
    ctx.current_org_id = None if ctx.is_platform_admin else 1
    ctx.org_role = None
    return ctx
